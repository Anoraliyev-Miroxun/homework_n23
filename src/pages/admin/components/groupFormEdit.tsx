...
import { useEditGroup } from "../services/mutation/useEditGroup";
...
interface FormProps {
    defaultValueData?: GroupDetail;
    closeModal: () => void;
    groupId: string;
}


export const GroupEditForm = ({ closeModal, defaultValueData, groupId }: FormProps) => {
    const client = useQueryClient();
    const { data, isLoading } = useTeacherList();
    const { mutate, isPending } = useEditGroup(groupId as string);

    const form = useForm<z.infer<typeof formSchema>>({
        resolver: zodResolver(formSchema),
        defaultValues: {
            startTime: defaultValueData?.data.startTime || "",
            endTime: defaultValueData?.data.endTime || "",
            durationInMonths: defaultValueData?.data?.durationInMonths?.toString() || "0",
            teacherId: defaultValueData?.data.teacherId || "",
        },
    });
